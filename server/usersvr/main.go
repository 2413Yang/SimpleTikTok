...
func Run() error {
	...
	if err := consulClient.Register(config.GetGlobalConfig().SvrConfig.Host, config.GetGlobalConfig().SvrConfig.Port,
		...
		log.Fatalf("consul.Register  error:", err)
		return fmt.Errorf("consul.Register  error:", err)
	}
	log.Info("Init Consul  Register success")

	// 启动
	log.Infof("TikTokLite.relation_svr listening on %s:%d", config.GetGlobalConfig().SvrConfig.Host, config.GetGlobalConfig().SvrConfig.Port)
	go func() {
